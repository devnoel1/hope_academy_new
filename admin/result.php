...
session_start();
  ...
  $cls_am = $handle->query("select * from class where cls_id = '$cls_id' ");
  
  $cls_arm = mysqli_fetch_assoc($cls_am);

  $checker = $_GET['checker'];
  if($cls_arm['cls_arm'] == "SS")
  {
  	return header("location:ss_result.php?std_id=$std_id&cls=$cls_id&term=$term&ses=$ses&sec=$sec&checker=$checker");
  }
...
<?php    
    ...
    ?>
    <span style="float: right"><button class="btn btn-success" onclick="printResult('#result')">Print</button></span>
    ...
    <div class="table-responsive-sm" id="result">
        <table class="table table-sm table-borderless"  width="100%">
            <tr>
                ...
                <td align="center">
                    <h4><b>HOPE ACADEMY HIGH SCHOOL</b></h4>
                    <span><b>JUNIOR SECONDARY SCHOOL TERMLY RESULT</b></span>
                    <h6></h6>
                    <h6>OMBI ONE, JOS ROAD LAFIA</h6>
                </td>
                <th align="center">
                     <img height="100"  width="100" style="margin: 5px 0px 0px 3px;" src="../img/logo1.png">   
                </th>
            </tr>
        </table>
        <table class="table table-borderless table-sm details" width="100%">
            <tr>
                ...
                <th class="text-uppercase"><b>Class:</b><strong> <?=$details['cls_name'];?></strong></th>
        ...
        <table class="table-sm table-bordered" width="100%">
            ...
            <?php
                $result = $handle->fetchQuery("select * from result join subject on subject.sub_id = result.sub_id 
                        ...
                        $check_rank = $handle->fetchQuery("select MAX(total) as highest,
                        Min(total) as lowest from result where std_id='' and cls_id='' and term_id='' and ses_id='' and section='' ");
...
<script type="text/javascript">
        ...
        function Popup(data) {
            var myWindow = window.open('', 'Result', 'height=1000,width=1000');
