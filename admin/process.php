...
if(isset($_GET['add_cls'])){
  ...
  $count = $handle->fetch("select * from reg_no_count") or die('Error'.mysqli_error($handle->con));
...
$handle->query("update reg_no_count set cont='$number' where id = '1' ") or die('Error'.mysqli_error($handle->con));
      ...
      //getting file data
       ...
       //requiredfile extention
		...
		//sending registered courses to database
            ...
            $check = $handle->numRows("select * from sub_reg where std_id = '$std_id' and section='$sec' and cls_id = '$cls' ");
	
            if($check > 0 ){
            	
                        $handle->query("delete from sub_reg where std_id = '$std_id' and section='$sec' and cls_id = '$cls'");
