...
<div class="container-fluid">
    <div class="table-responsive-sm">
            ...
            //getting ssubject details
              ...
              $cls_id = $_GET['cls_id'];
                $d = $handle->fetch("select *,subject.sub_name,subject.sub_id,class.cls_name,class.cls_id
                        ...
                        join class on class.cls_id = sub_comb.cls_id  
                        where sub_comb.sub_id = '$sub_id' and sub_comb.section='$sec' and sub_comb.cls_id = '$cls_id' ");
            ?>
            <tr>
                <td>Subject: <span class="text-capitalize"><?=$d['sub_name'];?></span></td>
                <td>Class: <span class="text-uppercase"><?=$d['cls_name'];?></span></td>
        ...
        <?php
                    $sub_id = $_GET['sub_id'];
            $view = $handle->fetchQuery("select *,student.std_id from result
                join student on student.std_id = result.std_id    
                where result.sub_id = '$sub_id' and result.section = '$sec'
                    and result.term_id = '$term' and result.ses_id ='$session' and result.cls_id = '".$_GET['cls_id']."'
                    ");
            $sno = 1;
            foreach ($view as $v){
            ?>
        ...
        <table class="table table-sm table-bordered" width="100%">
            ...
            $view = $handle->fetchQuery("select *,student.std_id,student.surname,student.other_name,student.middle_name
                ...
                where sub_reg.sub_id = '$sub_id' and sub_reg.cls_id ='$cls' and sub_reg.section = '$sec'");
            $sno = 1;
            for ($i=0;isset($view[$i]);$i++){
            ?>
            <tr>
                <td><?=$sno++;?></td>
                <td hidden=""><input type="hidden" value="<?=$view[$i]['std_id'];?>" name="std_id[]"></td>
